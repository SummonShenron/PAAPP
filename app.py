...
{{"action": "call_tool", "tool": "clear_sticky_note", "key": "Note Title to remove"}}"""
    ...
    # --- PATTERN D: NATIVE CONVERSATIONAL TOKEN STREAMER ---
    ...
    # --- PATTERN E: STREAM TOOL - ADD STICKY NOTE ---
    elif action_type == "call_tool" and tool_name == "add_sticky_note":
        tool_result = add_sticky_note(
            key=intent.get("key", "Note"),
            content=intent.get("content", "")
        )
        parsed_res = json.loads(tool_result)
        formatted_msg = f"**Sticky Note Added**\n\n{parsed_res['message']}"
